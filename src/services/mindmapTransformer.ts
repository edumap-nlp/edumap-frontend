...
/**
 * Insert expansion content under a specific node in markdown.
 */
export function insertExpansionInMarkdown(
  markdown: string,
  nodeId: string,
  expansionMarkdown: string,
  nodes: MindMapNode[]
): string {
  ...
  const data = node.data as MindMapNodeData
  const lines = markdown.split('\n')
  const targetLine = data.markdownLine

  if (targetLine === undefined || targetLine < 0 || targetLine >= lines.length) {
    return markdown + '\n' + expansionMarkdown
  }

  // Determine the depth of the target line
  const headingMatch = lines[targetLine].match(/^(#{1,6})\s+/)
  const depth = headingMatch ? headingMatch[1].length + 1 : 7

  // Format expansion lines with proper depth
  const expansionLines = expansionMarkdown
    .split('\n')
    .filter((l) => l.trim())
    .map((l) => {
      ...
      return `${'  '.repeat(depth - 7)}- ${clean}`
    })

  // Find the next sibling or end
  let insertAt = targetLine + 1
  while (insertAt < lines.length) {
    const lineHeading = lines[insertAt].match(/^(#{1,6})\s+/)
    if (lineHeading && lineHeading[1].length <= (headingMatch?.[1].length ?? 0)) break
    insertAt++
  }

  lines.splice(insertAt, 0, ...expansionLines)
  return lines.join('\n')
}
