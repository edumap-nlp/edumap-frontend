...
// [EduMap fix] 2026-04-23: MDEditor + its command orchestrator are no
// longer imported here — the previous inline markdown toolbar and the
// underline command were part of an editor surface that's been stubbed
// out (the outline renders from the parsed node tree instead). Dropping
// the imports shaves a dependency off this bundle chunk.
...
export default function MarkdownEditorPanel({
  ...
  //
  // [EduMap fix] 2026-04-23: Expand All / Collapse All were moved out of
  // this header into the TopNav so every button lives in one place.
  ...
  return (
    <section className="flex flex-col h-full border-r border-surface-border bg-panel min-w-[420px] max-w-[520px]">
      ...
      <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
          ...
          <div className="border-t border-surface-border max-h-[600px] overflow-y-auto shrink-0">
            <div className="px-4 py-2 bg-slate-50/80 border-b border-surface-border sticky top-0 z-10">
              ...
              <p className="text-[10px] text-slate-500 mt-0.5">
                Click ▶ to expand · click a row to locate it in the mind map · Expand/Collapse in the top bar
              </p>
            </div>
            <div className="py-1">
              {nodeIndex.map((item) => {
        ...
        {/* [EduMap fix] 2026-04-23: Save Modifications and Copy Markdown
            buttons were moved to the TopNav (under a single "Save"
            dropdown) so every action lives in one place. */}
      </div>
    </section>
  )
}
...
function TagBadgeSmall({ tag }: { tag: NodeTag }) {
  // [EduMap fix] 2026-04-23: Handle arbitrary tag strings — preset colors
  // for the known slugs, neutral slate for anything custom. Labels come
  // from a Title-Cased slug when the preset lookup misses.
  const presets: Record<string, { cls: string; label: string }> = {
    hard: { cls: 'bg-yellow-200 text-yellow-900', label: 'Hard' },
    easy: { cls: 'bg-emerald-100 text-emerald-800', label: 'Easy' },
  ...
  const cls = preset?.cls ?? 'bg-slate-100 text-slate-600'
  const label =
    preset?.label ??
    tag
      .split('-')
      ...
      .join(' ')
  return (
    <span className={`inline-flex items-center px-1.5 py-0.5 rounded text-[9px] font-semibold ${cls} shrink-0`}>
      {label}
    </span>
  )
}

// [EduMap fix] 2026-04-23: Removed unused ToolbarButton helper along with
// the commented-out MDEditor surface. The outline-only layout doesn't
// need it; reintroduce if/when an inline markdown editor is wired back
// in.
